...
class Ejecucion {
    ...
    recorrer(nodo) {
        ...
        if (this.soyNodo('INSTRUCCION_PORSI', nodo)) {
            switch (nodo.hijos.length) {
                //IF
                case 1:
                ...
                case 2:
                    //IF ELSE -> PORSI PORSIACA
                    if (this.soyNodo('PORSI', nodo.hijos[0]) && this.soyNodo('PORSIACA', nodo.hijos[1])) {
                    ...
                    //IF LISTA_ELSE_IF -> PORSI LISTA_PORSIACA_PORSI
                    if (this.soyNodo('PORSI', nodo.hijos[0]) && this.soyNodo('LISTA_PORSIACA_PORSI', nodo.hijos[1])) {
                        const inst_if = this.recorrer(nodo.hijos[0]);
        ...
        if (this.soyNodo('PORSIACA_PORSI', nodo)) {
        ...
        if (this.soyNodo('LISTA_PORSIACA_PORSI', nodo)) {
            const lista = [];
            nodo.hijos.forEach((nodoHijo) => {
        ...
        if (this.soyNodo('HACER_BUCLESI', nodo)) {
        ...
        //ASIGNACION_FOR -> ASIGNACION_FOR
        if (this.soyNodo('ASIGNACION_REPETIDOR', nodo)) {
            const id = nodo.hijos[0];
            switch (nodo.hijos.length) {
                // id TIPO_IGUAL EXP
                case 3:
        ...
        //FOR -> REPETIDOR
        if (this.soyNodo('REPETIDOR', nodo)) {
            const condicion = this.recorrer(nodo.hijos[3]);
        ...
        //FOR_OF -> REPETIDOR_OF
        if (this.soyNodo('REPETIDOR_OF', nodo)) {
        ...
        //FOR_IN -> REPETIDOR_IN
        if (this.soyNodo('REPETIDOR_IN', nodo)) {
