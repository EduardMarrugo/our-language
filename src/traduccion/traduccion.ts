...
export class Traduccion {
  ...
  recorrer(nodo: any, e: Entorno): any {
      ...
      switch (nodo.hijos.length) {
        ...
        case 9:
          // function test() : TIPO { INSTRUCCIONES }
          {
            ...
            const fn = new Funcion({id: nombre});
            if(nombre != id){
              fn.setIdNuevo(id);
            }
            e.setFuncion(fn);
            ...
            //Si tiene funcion anidada
              ...
              const entorno = new Entorno(e, nombre);
        ...
        case 11:
          {
            ...
            if (!this.tengoFuncionAnidada(nodo.hijos[9])) {
              codigoAux += this.recorrer(nodo.hijos[9], new Entorno(e));
              codigoAux += `}\n\n`;
            }
            //Si tiene funcion anidada
            else {
              ...
              //Realizo el recorrido para las funciones anidadas
              nodo.hijos[9].hijos.forEach((nodoHijo: any) => {
                if (this.soyNodo('DECLARACION_FUNCION', nodoHijo)) {
                  codigoFn += this.recorrer(nodoHijo, entorno) + '\n';
                }
              });
    ...
    // else if (this.soyNodo('INSTRUCCION_IF', nodo)) {
      else if (this.soyNodo('INSTRUCCION_PORSI', nodo)) {
      let codigoAux = '';
      nodo.hijos.forEach((nodoHijo: any) => {
        if (nodoHijo instanceof Object) {
          codigoAux += this.recorrer(nodoHijo, e);
    ...
    // else if (this.soyNodo('RETURN', nodo)) {
      else if (this.soyNodo('DEVUELVE', nodo)) {
      switch (nodo.hijos.length) {
        // return punto_coma
        case 2:
          return 'devuelve;'
    ...
    //WHILE -> BLUCESI
    else if (this.soyNodo('BUCLESI', nodo)) {
      // while par_izq EXP par_der llave_izq INSTRUCCIONES llave_der ->  buclesi par_izq EXP par_der llave_izq INSTRUCCIONES llave_der
      const exp = this.recorrer(nodo.hijos[2], e);
    ...
    //DO_WHILE -> HACER_BUCLESI
    else if (this.soyNodo('HACER_BUCLESI', nodo)) {
      // do llave_izq INSTRUCCIONES llave_der while par_izq EXP par_der punto_coma ->  hacer llave_izq INSTRUCCIONES llave_der buclesi par_izq EXP par_der punto_coma
      const entorno = new Entorno(e);
      const instrucciones = this.recorrer(nodo.hijos[2], entorno);
      const exp = this.recorrer(nodo.hijos[6], e);
      return `hacer{\n${instrucciones}}buclesi(${exp});`;
    }

    //FOR ->  REPETIDOR
    else if (this.soyNodo('REPETIDOR', nodo)) {
      switch (nodo.hijos.length) {
        case 10:
          ...
          if (this.soyNodo('DECLARACION_VARIABLE', nodo.hijos[2])) {
            ...
            return `repetidor(${dec_var} ${exp}; ${asig_for}){\n${instrucciones}}`;
          }
          ...
          if (this.soyNodo('ASIGNACION', nodo.hijos[2])) {
            ...
            return `repetidor(${asig} ${exp}; ${asig_for}){\n${instrucciones}}`;
          }
      }
    }

    //FOR_OF -> REPETIDOR_OF
    else if (this.soyNodo('REPETIDOR_OF', nodo)) {
      ...
      const instrucciones = this.recorrer(nodo.hijos[8], new Entorno(entorno));
      return `repetidor(${tipo_dec_var} ${id} of ${exp}){\n${instrucciones}}`;
    }

    //FOR_IN -> REPETIDOR_IN
    else if (this.soyNodo('REPETIDOR_IN', nodo)) {
      ...
      const exp = this.recorrer(nodo.hijos[5], entorno);
      const instrucciones = this.recorrer(nodo.hijos[8], new Entorno(entorno));
      return `repetidor(${tipo_dec_var} ${id} in ${exp}){\n${instrucciones}}`;
    }
    ...
    //ASIGNACION_FOR -> ASIGNACION_REPETIDOR
    else if (this.soyNodo('ASIGNACION_REPETIDOR', nodo)) {
      const id = nodo.hijos[0];
      switch (nodo.hijos.length) {
    ...
    //DEFAULT
      ...
      const instrucciones = this.recorrer(nodo.hijos[2], new Entorno(e));
      return `default:\n${instrucciones}`;
    }
    ...
    else if (this.soyNodo('PORSI', nodo)) {
